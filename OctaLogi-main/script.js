...
const STANDARD_SECTORS = [
  ...
  "Administrative Hub General",
  "Administrative Hub Legal",
  "Administrative Hub People and Culture",
  "Business Commercial",
  "Business Marketing",
  "Business Pitch Development",
  "Innovation General",
  "Innovation Research and Development Game Development",
  "Innovation Research and Development Information Technology",
  "Innovation Research and Development Pipeline",
  "Operational 3D Development General",
  "Operational 3D Development Simulation",
  "Operational Animation 2D Animation",
  "Operational Animation 3D Animation",
  "Operational Animation Editing",
  "Operational Animation Rigging",
  "Operational Animation Storyboard",
  "Operational Creative Direction",
  "Operational Creative Project Management",
  "Operational General",
  "Operational Post Production Still",
  "Operational Post Production Video",
  "Operational Project Production",
  "Operational Set Production",
  "Operational Visual Development",
  "Strategy Executive Management"
];

const GLPI_CC_DROPDOWN_MAP = {
  "1": "CC.001.000.000",
  "2": "CC.001.001.000",
  "3": "CC.001.002.000",
  "4": "CC.001.003.000",
...
function extractGlpiCentroCusto(u) {
  if (!u || typeof u !== 'object') return '';

  // 1. Inspecionar o campo ID 76878 (campo do plugin "USUARIO - CENTRO DE CUSTO" no GLPI)
  const rawCc = u['76878'] || u[76878] || u['Plug-ins - USUARIO - CENTRO DE CUSTO'] || u['Plug-ins - USUÁRIO - CENTRO DE CUSTO'] || u['USUARIO - CENTRO DE CUSTO'] || u['USUÁRIO - CENTRO DE CUSTO'];

  if (rawCc !== undefined && rawCc !== null && rawCc !== '') {
    const val = parseGlpiValue(rawCc);
    if (val) {
      let idStr = val;
      if (typeof val === 'string' && val.startsWith('[')) {
        try {
          ...
          if (Array.isArray(parsedArr) && parsedArr.length > 0) {
            idStr = String(parsedArr[0]);
          }
        } catch (e) {
          idStr = val.replace(/[\[\]"'\s]/g, '');
        }
      }
      idStr = String(idStr).replace(/[\[\]"'\s]/g, '');

      if (GLPI_CC_DROPDOWN_MAP[idStr]) {
        return GLPI_CC_DROPDOWN_MAP[idStr];
      }

      const match = String(val).match(/(CC[\.\-_]\d[\d\.\-_]*)/i);
      if (match && match[1]) {
  ...
  // 2. Chaves conhecidas ou busca dinâmica por "centro" + "custo"
  for (const key of Object.keys(u)) {
    const keyLower = key.toLowerCase();
    if (key === '76878' || (keyLower.includes('centro') && keyLower.includes('custo')) || (keyLower.includes('cost') && keyLower.includes('center'))) {
      const val = parseGlpiValue(u[key]);
      if (val) {
        let idStr = String(val).replace(/[\[\]"'\s]/g, '');
        if (GLPI_CC_DROPDOWN_MAP[idStr]) {
          return GLPI_CC_DROPDOWN_MAP[idStr];
        }
        const match = String(val).match(/(CC[\.\-_]\d[\d\.\-_]*)/i);
  ...
  const pluginObj = u.plugin_fields || u._plugin_fields || u.plugins;
  if (pluginObj && typeof pluginObj === 'object') {
    for (const key of Object.keys(pluginObj)) {
      const keyLower = key.toLowerCase();
      if ((keyLower.includes('centro') && keyLower.includes('custo')) || (keyLower.includes('cost') && keyLower.includes('center'))) {
        const val = parseGlpiValue(pluginObj[key]);
        if (val) {
          let idStr = String(val).replace(/[\[\]"'\s]/g, '');
          if (GLPI_CC_DROPDOWN_MAP[idStr]) {
            return GLPI_CC_DROPDOWN_MAP[idStr];
  ...
  // 3. Inspecionar todas as propriedades do objeto em busca do formato de código (ex: CC.005.002.000)
  for (const key of Object.keys(u)) {
    const val = parseGlpiValue(u[key]);
    if (val && typeof val === 'string') {
      const match = val.match(/(CC[\.\-_]\d[\d\.\-_]*)/i);
      if (match && match[1]) {
        return match[1].toUpperCase();
      }
    }
  ...
  if (u.centro_custo) return String(u.centro_custo).trim();
  if (u.centroCusto) return String(u.centroCusto).trim();

  return '';
}

function extractGlpiSector(u) {
  if (!u || typeof u !== 'object') return 'Operational General';

  const candidates = [
    u.completename_group,
    u.groups_id,
    u.group_name,
    u.group,
    u.groups,
  ...
  for (const cand of candidates) {
    const val = parseGlpiValue(cand);
    if (val && val !== '0' && val !== 'null') {
      const matched = findMatchingSetor(val);
  ...
  for (const key of Object.keys(u)) {
    const keyLower = key.toLowerCase();
    if (keyLower.includes('setor') || keyLower.includes('grupo') || keyLower.includes('group') || keyLower.includes('department') || keyLower.includes('departamento')) {
      const val = parseGlpiValue(u[key]);
      if (val && val !== '0' && val !== 'null') {
        const matched = findMatchingSetor(val);
        if (matched) return matched;
...
function parseGlpiValue(val) {
  if (val === null || val === undefined) return '';
  if (typeof val === 'string' || typeof val === 'number') {
    return String(val).trim();
  }
  if (typeof val === 'object') {
    const nameVal = val.completename || val.name || val.text || val.value;
    if (nameVal) return String(nameVal).trim();
  }
  return '';
}

function updateCentroCustoFromSetor() {
  // Removido: Centro de custo é puxado diretamente do GLPI via API
}
...
async function fetchGlpiUsersViaGet(query = '') {
  ...
  if (baseUrl && GLPI_CONFIG.APP_TOKEN) {
    try {
      ...
      const tokenParam = (sessionToken ? `&session_token=${encodeURIComponent(sessionToken)}` : (userToken ? `&user_token=${encodeURIComponent(userToken)}` : ''));
      const searchUrl = `${baseUrl}/search/User?expand_dropdowns=true&is_deleted=0&range=0-1000&forcedisplay[0]=1&forcedisplay[1]=2&forcedisplay[2]=9&forcedisplay[3]=34&forcedisplay[4]=70&forcedisplay[5]=76878&app_token=${encodeURIComponent(appToken)}${tokenParam}`;
...
function processRawGlpiUsers(data) {
  ...
  if (rawList[0]) {
    console.log("🔍 GLPI Raw User Sample Data:", rawList[0]);
  }
  ...
  return rawList.map(u => {
    ...
    const matchedSetor = extractGlpiSector(u);
    const cc = extractGlpiCentroCusto(u);
...
function findMatchingSetor(rawName) {
  ...
  for (const setorName of STANDARD_SECTORS) {
    if (setorName.toLowerCase() === lower) {
      return setorName;
    }
  }

  const aliasMap = [
    { keywords: ['finance', 'finanças', 'financas', 'financeiro'], target: 'Administrative Hub Finance' },
    { keywords: ['legal', 'jurídico', 'juridico'], target: 'Administrative Hub Legal' },
    { keywords: ['people', 'culture', 'rh', 'hr', 'recursos humanos', 'povo e cultura'], target: 'Administrative Hub People and Culture' },
    { keywords: ['administrative hub general', 'adm geral', 'administrativo geral', 'admin general'], target: 'Administrative Hub General' },
    { keywords: ['commercial', 'comercial', 'vendas', 'sales'], target: 'Business Commercial' },
    { keywords: ['marketing', 'mkt'], target: 'Business Marketing' },
    { keywords: ['pitch'], target: 'Business Pitch Development' },
    { keywords: ['game', 'games', 'jogos'], target: 'Innovation Research and Development Game Development' },
    { keywords: ['technology', 'tecnologia', 'information technology', 'ti', 'it', 'ti/it'], target: 'Innovation Research and Development Information Technology' },
    { keywords: ['pipeline'], target: 'Innovation Research and Development Pipeline' },
    { keywords: ['innovation general', 'inovação geral', 'inovacao geral'], target: 'Innovation General' },
    { keywords: ['simulation', 'simulação', 'simulacao'], target: 'Operational 3D Development Simulation' },
    { keywords: ['3d general', '3d geral'], target: 'Operational 3D Development General' },
    { keywords: ['2d'], target: 'Operational Animation 2D Animation' },
    { keywords: ['rigging', 'rig'], target: 'Operational Animation Rigging' },
    { keywords: ['storyboard'], target: 'Operational Animation Storyboard' },
    { keywords: ['editing', 'edição', 'edicao'], target: 'Operational Animation Editing' },
    { keywords: ['3d animation', 'animação 3d', 'animacao 3d'], target: 'Operational Animation 3D Animation' },
    { keywords: ['creative direction', 'direção criativa', 'direcao criativa'], target: 'Operational Creative Direction' },
    { keywords: ['creative project', 'gestão criativa', 'gestao criativa'], target: 'Operational Creative Project Management' },
    { keywords: ['still', 'post production still', 'pós produção still', 'pos producao still'], target: 'Operational Post Production Still' },
    { keywords: ['video', 'vídeo', 'post production video', 'pós produção video'], target: 'Operational Post Production Video' },
    { keywords: ['set production', 'produção de set', 'producao de set'], target: 'Operational Set Production' },
    { keywords: ['project production', 'produção de projeto', 'producao de projeto'], target: 'Operational Project Production' },
    { keywords: ['visual development', 'visdev', 'desenvolvimento visual'], target: 'Operational Visual Development' },
    { keywords: ['strategy', 'executive', 'diretoria', 'executiva', 'gestão executiva'], target: 'Strategy Executive Management' }
  ];
  ...
  for (const setorName of STANDARD_SECTORS) {
    const sLower = setorName.toLowerCase();
...
function selectGlpiUserForMainForm(user) {
  ...
  if (campoSetor) {
    setSelectValueSafely(campoSetor, user.setor);
    if (user.isCustom) {
...
function selectGlpiUserForEditForm(user) {
  ...
  if (editCampoSetor) {
    setSelectValueSafely(editCampoSetor, user.setor);
    if (user.isCustom) {
...
function setSelectValueSafely(selectEl, value) {
  ...
  for (let i = 0; i < selectEl.options.length; i++) {
    if (selectEl.options[i].value.trim().toLowerCase() === targetVal) {
